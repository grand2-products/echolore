...
- Keep the app env files explicit. If you change `WEB_PORT`, `API_PORT`, `DB_PORT`, or `LIVEKIT_PORT`, also update related localhost URL values such as `ECHOLORE_PUBLIC_API_URL`, `CORS_ORIGIN`, `ROOM_AI_API_BASE_URL`, `LIVEKIT_HOST`, and `DATABASE_URL` in the same app env file.
