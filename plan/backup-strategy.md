...
```
# /etc/cron.d/corp-internal-backup
...
```bash
rsync -az --delete /opt/wiki/backups/ backup-user@backup-host:/backups/corp-internal/
```
...
```bash
rclone sync /opt/wiki/backups/ remote:corp-internal-backups/
```
...
```bash
# DB リストア
docker exec -i corp-internal-db \
...
# Install backup cron
cat > /etc/cron.d/corp-internal-backup <<'CRON'
0 3 * * * deploy /opt/wiki/scripts/backup.sh >> /opt/wiki/backups/backup.log 2>&1
CRON
```
