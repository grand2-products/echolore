...
5. **課金を有効化**: Cloud Console → **「お支払い」** → 請求先アカウントをプロジェクトにリンク

> **課金の有効化は必須です。** Vertex AI・Cloud Storage・Speech API など、ほぼすべての GCP サービスは課金アカウントが紐付いていないと利用できません。無料枠の範囲内でも課金の有効化が必要です。
...
2. 名前: `echolore`（任意）
3. ロールを付与（使用する機能に応じて選択）:
   - **「Vertex AI ユーザー」**（`roles/aiplatform.user`）— LLM 推論
   - **「Speech クライアント」**（`roles/speech.client`）— 音声文字起こし・音声合成（Speech-to-Text / Text-to-Speech を使う場合）
   - **「Storage オブジェクト管理者」**（`roles/storage.objectAdmin`）— GCS ファイルストレージ（[A-5](#a-5-google-cloud-storagegcs) を使う場合）
4. 「完了」をクリック
...
**Embedding**セクション（Vertex AI 経由で Embedding を使う場合）:

| フィールド | 値 |
|---|---|
| Embedding プロバイダ | `Vertex AI` を選択 |
| Embedding モデル | 使用モデル（デフォルト: `gemini-embedding-2-preview`） |

> Embedding プロバイダを `Vertex AI` にすると、LLM プロバイダセクションで設定した Project / Location を使用し、アプリケーションデフォルト認証情報（ADC）で認証します。Gemini API キーは不要です。
> `Gemini API` のままにした場合は従来通り API キーで認証します。
...
4. ストレージクラス: **Autoclass**（推奨 — アクセス頻度に応じて自動的に最適なクラスへ移行しコストを削減）
5. アクセス制御: **「均一」** を選択
6. 「作成」をクリック

> **注意:** Autoclass は一度有効にするとバケット単位で無効化できません。手動でストレージクラスを管理したい場合は Standard を選択してください。

#### サービスアカウントの権限確認

A-4 でサービスアカウント作成時に `roles/storage.objectAdmin` を付与済みであれば、追加作業は不要です。

まだ付与していない場合は、以下の手順で追加してください:
...
| SSO + AI（Z.ai） | A-1 + A-2 + A-3（Embedding 用）+ [付録 B](#付録-b-zaiglmセットアップ) | 不要（API キーのみ） |
| SSO + AI + GCS（エンタープライズ） | A-1 + A-2 + A-4 + A-5 | Vertex AI ユーザー + Storage オブジェクト管理者 |
...
>
> Vertex AI 構成では、Embedding も Vertex AI 経由で利用できます。管理画面の Embedding セクションでプロバイダを `Vertex AI` に変更してください（デフォルトは Gemini API）。
...
| `google_project_iam_member` | IAM ロール付与（Storage Object Admin, Vertex AI User, Speech Client） |
