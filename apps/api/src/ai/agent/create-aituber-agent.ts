...
export function createAituberAgent(
  ...
  const systemMessage = [
    ...
    "",
    "## ツールの使い方",
    "- 視聴者の質問に答えるために必要なら、wiki_search で社内ドキュメントを探してください。",
    "- 検索で見つかったページの内容が不足する場合のみ、wiki_read_page で全文を取得してください。",
    "- 「最近の記事は？」「何が書いてある？」のような探索系には wiki_list_pages を使ってください。",
    "- ツール検索の結果はあなた（AIキャラクター）が読み、その内容を参考にキャラクターとして簡潔に話してください。",
    "- ツールが返す本文をそのまま読み上げないでください — マークダウンも貼り付けないでください。",
    "- 視聴者が read 権限を持つページしか検索できません。見つからない場合は素直にそう伝えてください。",
    contextBlock,
  ]
    .filter(Boolean)
    .join("\n");
