...
filesRoutes.get("/", withErrorHandler(async (c) => {
...
}, "FILES_LIST_FAILED", "Failed to fetch files"));

// GET /api/files/:id - Get file metadata
filesRoutes.get("/:id", withErrorHandler(async (c) => {
...
filesRoutes.post("/upload", withErrorHandler(async (c) => {
  const user = c.get("user");
  if (!user?.id) {
...
filesRoutes.get("/:id/download", withErrorHandler(async (c) => {
  const { id } = c.req.param();
...
filesRoutes.delete("/:id", withErrorHandler(async (c) => {
  const { id } = c.req.param();
