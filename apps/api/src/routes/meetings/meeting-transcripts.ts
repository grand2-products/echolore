...
meetingTranscriptRoutes.post("/:id/transcripts", zValidator("json", createTranscriptSchema), withErrorHandler(async (c) => {
...
}, "MEETING_TRANSCRIPT_CREATE_FAILED", "Failed to add transcript"));

// GET /api/meetings/:id/realtime/transcripts
meetingTranscriptRoutes.get("/:id/realtime/transcripts", withErrorHandler(async (c) => {
  const { id } = c.req.param();
...
meetingTranscriptRoutes.post(
  ...
  }, "MEETING_REALTIME_TRANSCRIPT_UPSERT_FAILED", "Failed to upsert realtime transcript")
);
