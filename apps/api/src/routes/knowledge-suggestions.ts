...
knowledgeSuggestionsRoutes.get(
  "/",
  withErrorHandler("KNOWLEDGE_LIST_FAILED", "Failed to list knowledge suggestions"),
  async (c) => {
...
knowledgeSuggestionsRoutes.get(
  "/:id",
  withErrorHandler("KNOWLEDGE_DETAIL_FAILED", "Failed to get knowledge suggestion"),
  async (c) => {
...
knowledgeSuggestionsRoutes.post(
  "/:id/approve",
  withErrorHandler("KNOWLEDGE_APPROVE_FAILED", "Failed to approve knowledge suggestion"),
  async (c) => {
    const { id } = c.req.param();
    const user = c.get("user");
...
knowledgeSuggestionsRoutes.post(
  "/:id/reject",
  withErrorHandler("KNOWLEDGE_REJECT_FAILED", "Failed to reject knowledge suggestion"),
  async (c) => {
    const { id } = c.req.param();
    const user = c.get("user");
