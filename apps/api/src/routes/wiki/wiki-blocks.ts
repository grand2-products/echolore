...
wikiBlockRoutes.post("/blocks", zValidator("json", createBlockSchema), withErrorHandler(async (c) => {
...
}, "WIKI_BLOCK_CREATE_FAILED", "Failed to create block"));

wikiBlockRoutes.delete("/blocks/:id", withErrorHandler(async (c) => {
...
}, "WIKI_BLOCK_DELETE_FAILED", "Failed to delete block"));

wikiBlockRoutes.put("/blocks/:id", zValidator("json", updateBlockSchema), withErrorHandler(async (c) => {
  const { id } = c.req.param();
  const data = c.req.valid("json");
...
}, "WIKI_BLOCK_UPDATE_FAILED", "Failed to update block"));
