...
wikiTrashRoutes.get("/trash", withErrorHandler(async (c) => {
...
}, "WIKI_TRASH_LIST_FAILED", "Failed to fetch trash"));

wikiTrashRoutes.post("/trash/:id/restore", withErrorHandler(async (c) => {
...
}, "WIKI_PAGE_RESTORE_FAILED", "Failed to restore page"));

wikiTrashRoutes.delete("/trash/:id", withErrorHandler(async (c) => {
  const { id } = c.req.param();
  const user = c.get("user");
...
}, "WIKI_PAGE_DELETE_FAILED", "Failed to permanently delete page"));
