...
async function authorizePagePermissionAccess(c: Parameters<typeof authorizePageResource>[0], pageId: string) {
  ...
  await writeAuditLog({
    ...
    resourceId: pageId,
    metadata: { reason: allowed ? (user.role === UserRole.Admin ? "admin" : "owner") : "not-owner" },
...
wikiPermissionRoutes.get("/:id/permissions", withErrorHandler(async (c) => {
  const { id } = c.req.param();
  const { error } = await authorizePagePermissionAccess(c, id);
  if (error) return error;

  return c.json(await getPagePermissionsDetail(id));
}, "WIKI_PAGE_PERMISSIONS_FETCH_FAILED", "Failed to fetch page permissions"));
...
wikiPermissionRoutes.put(
  ...
  }, "WIKI_PAGE_PERMISSIONS_SET_FAILED", "Failed to set page permissions")
);

wikiPermissionRoutes.delete("/:id/permissions/groups/:groupId", withErrorHandler(async (c) => {
...
}, "WIKI_PAGE_PERMISSION_DELETE_FAILED", "Failed to remove permission"));

wikiPermissionRoutes.get("/:id/permissions/inherit", withErrorHandler(async (c) => {
  const { id } = c.req.param();
  const { error } = await authorizePagePermissionAccess(c, id);
  if (error) return error;
...
}, "WIKI_PAGE_INHERITANCE_FETCH_FAILED", "Failed to fetch inheritance"));
...
wikiPermissionRoutes.put(
  ...
  }, "WIKI_PAGE_INHERITANCE_SET_FAILED", "Failed to set inheritance")
);
